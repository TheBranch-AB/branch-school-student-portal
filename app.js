...
async function loadTodaysCalendar() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Calendar API HTTP ${response.status}`);
    }

    const data = await response.json();
    console.log("Today's calendar events:", data.items || []);

    const scheduleBtn = document.querySelector(".schedule-btn");
    const count = (data.items || []).length;

    scheduleBtn.textContent =
      count === 1 ? "1 Event Today ›" : `${count} Events Today ›`;
  } catch (error) {
...
function requestCalendarAccess(event) {
  ...
  calendarTokenClient.requestAccessToken({
    prompt: calendarAccessToken ? "" : "consent"
  });
}
